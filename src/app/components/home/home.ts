...
export class Home implements AfterViewInit, OnDestroy {
  ...
  /** Bound to [class.globe-exit] in the template */
  isLeaving = false;
  ...
  private texPixels!: Uint8ClampedArray;
  ...
  // ── Listen for router navigation to trigger exit animations ─────────────────
  private listenForNavigation(): void {
    this.routerSub = this.router.events
      .pipe(filter(e => e instanceof NavigationStart))
      .subscribe(() => this.triggerExit());
  }

  /**
   * Call this before any programmatic navigation if you want exit animations.
   * The router guard / RouterLink will fire NavigationStart which calls this
   * automatically; but for buttons using (click) you can also call it directly.
   */
  private triggerExit(durationMs = 500): void {
    this.isLeaving = true;
    // Add .exit class to each animatable hero element
    const targets = this.el.nativeElement.querySelectorAll(
      '.badge, .hero-title, .hero-sub, .description, .buttons'
    );
    targets.forEach((el: HTMLElement) => el.classList.add('exit'));
  }
  ...
  private initScrollAnimations(): void {
    ...
    const elements: NodeListOf<HTMLElement> =
      this.el.nativeElement.querySelectorAll(animatableSelectors);
  ...
  private drawSphere(): void {
    ...
    const sX = -0.5345, sY = 0.7682, sZ = 0.3563;
  ...
  // ── Count-up (triggered by scroll observer) ──────────────────────────────────
